...
- **Ten known gaps stay visible as expected failures.** Three concern backend evidence handling (critical mismatched-document attribution/quarantine, closing evidence graded FAIL/UNKNOWN, and empty or unknown evidence requirements); three concern the legacy transport path (same-millisecond cursors, bursts above 200 events, and HTTP 200 responses carrying `ok:false`); four concern the legacy dashboard (blood-pressure tile source, historical-green wording, atomic snapshots, and reset scoping). The enrolled cockpit replaces the four legacy-dashboard paths; the other six remain demo limitations. `SANAD_GATE1_STRICT=1` turns all ten into ordinary failing tests so none can be mistaken for a pass. This is one reason the repository and live service are synthetic-only and not clinically ready.
...
- **A blood pressure written in words is not read as one.** `core/vitals.parse` accepts a bare `138/88` and nothing else, which is deliberate and is why a reading filed by code is only ever a reading the patient sent on its own. The scanner that finds numbers inside prose (`core/vitals.find_all`) understands the same slash form and no other, so a patient who writes "138 over 88" has no reading filed against his monitoring loop and those two numbers are never graded against the three thresholds. The phrasings patients actually use are the next thing to widen here.
- **The Resolver's cost route needs an answer it never asks for.** The availability and transport routes ask the patient for his area before they search. The cost route asks only whether a public hospital lab would work, and then searches Places for one, which needs an area. For a patient with no area on his record that search cannot run, so the barrier goes to the doctor with the question already asked and the attempt printed on the card. That is the fail-closed direction and it is also a second question the route should be asking.
